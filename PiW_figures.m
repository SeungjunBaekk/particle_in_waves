...
for wI = 1:numel(waves)
    ...
    for pI = 1:numel(parts)
        ...
        % -----------------------------------------------------------------
        % 1) Load data: slip_dn_results_*.mat
        % -----------------------------------------------------------------
...
% legend(ax_leg, [h_leg_ens(:); h_leg_trial(:)],'FontSize',16,'Interpreter','latex')


% 기존 legend 호출을 아래처럼 바꾸기
...
for sp = 1:9
    if sp == 3, continue; end              % legend subplot은 건드리지 않음
    ax = ax_profile(sp);
    ch = get(ax,'Children');
    ...
    % Children 배열에서 ensemble 객체를 앞쪽으로 재배치
    % (앞쪽이 화면상 맨 위에 그려짐)
...
% exportgraphics(fig_profile_all,'E:\2.ParticlesInWave\Manuscript\figures\all_profiles.pdf')
outpdf = 'E:\2.ParticlesInWave\Manuscript\figures\all_profiles.pdf';

set(fig_profile_all,'Renderer','painters');   % 벡터 렌더러 강제

% exportgraphics(fig_profile_all, outpdf, ...
%     'ContentType','vector', ...               % 벡터 강제
%     'BackgroundColor','none');                % (선택) 배경 투명
%% ----------------- Datatip settings (optional) --------------------------
dcm = datacursormode(fig_profile_all);
dcm.Enable        = 'on';
dcm.SnapToDataVertex = 'on';
dcm.DisplayStyle  = 'datatip';
dcm.UpdateFcn     = @(~,evt) ptvTip(evt);   % use local function below
...
% Profiles vs y (load legacy files alongside if needed)
y_centers = cell(wN,pN);
...
for wI = 1:wN
  ...
  for pI = 1:pN
    ...
    S = load(fullfile(filepath, sprintf('%s%s_vs_avg_unit.mat', wave, partinfo)));
    ...
    % Robust mean of instantaneous v_avg (for outlier removal, if needed)
    x   = v_avg{wI,pI};
    x   = x(~isnan(x));
    med = median(x);
    mad0 = median(abs(x - med));
    sigma = 1.4826 * mad0;
    id_keep = abs(x-med) <= 3*sigma;
    x_f = x(id_keep);
    v_slip(wI,pI) = mean(x_f);
  end
end
...
% pick ONE particle like in all_profiles.pdf
...
for wI = 1:numel(waves)
    ...
    for j = 1:case_tot
        ...
        % --------- TRIMMING: longest "core" segment (same logic as your script) ----------
        winN = max(5, ceil(0.5*T/dt));
        finite_mask = isfinite(u_slip) & isfinite(v_slip) & isfinite(phi_std) & isfinite(t) & isfinite(vs_norm);

        movstd_vs = movstd(vs_norm, winN, 'omitnan');
        mad_vs    = mad(vs_norm, 1);
        center_vs = median(vs_norm, 'omitnan');
        core_mask = finite_mask & (movstd_vs < 0.10) & (abs(vs_norm - center_vs) <= 3*mad_vs);

        dmask = diff([false; core_mask(:); false]);
        rs = find(dmask==1);  re = find(dmask==-1)-1;

        if isempty(rs)
            idx_keep = find(finite_mask);
        else
            [~,imax] = max(re - rs + 1);
            idx_keep = rs(imax):re(imax);
        end

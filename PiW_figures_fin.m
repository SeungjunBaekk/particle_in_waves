...
for wI = 1:wN
    ...
    for pI = 1:pN
        ...
        for j = 1:case_tot
            h_trial = plot(ax_this, ensemble_v_norm(j,:), y_sign* k_y_centers, '--', ...
                'Color', c_trial, ...
                'Marker', m_wave, ...
                'MarkerSize', 3, ...
                'LineWidth', 0.8, ...
        ...
        h_ens = errorbar(ax_this, ensemble_binned_v_norm, y_sign* k_y_centers, ...
        ...
        if isfinite(ky_tau5)
            ...
            h_tau = plot(ax_this, xl, y_sign*[ky_tau5 ky_tau5], '-.', ...
                'Color', c_wave, ...
                'LineWidth', 1.2, ...
                'HandleVisibility','off');
...
% [r2] revised for reviewer comment R3-2(a) (figure legibility):
%   - measurements drawn with errorbar objects (white-filled markers)
%   - D22 prediction drawn with a DISTINCT LINE STYLE per wave case,
%     so the curves remain distinguishable even in grayscale print
%   - programmatic zoom inset in every panel for the near-unity region;
%     the inset window is marked by a grey box in the main axes
%     (removes the need for heavy external editing; only cosmetic
%     touch-up should remain)
%   - log x-scaling removed (range is only [0.8 1.2])
%   - exported as a true vector PDF
% ------------------------------------------------------------------------
inset_xl = 0.995; inset_xr = 1.005;
inset_yl = -0.5;  inset_yr = 0;

ls_wI  = {'-.','-.','-.'};     % D22 line style per wave (W1, W2, W3)
lw_d22 = 1.8;

fig_vz = figure('Color','w');
fig_vz.Position = [50 50 1600 1200];
set(fig_vz,'Renderer','painters');
...
for pI = 1:pN
    ...
    for wI = 1:wN
        ...
        % measurements: one errorbar object per (wave, particle)
        errorbar(axp, x, y, dy, dy, dx, dx, m_wI{wI}, ...
            'MarkerSize', 6, 'MarkerFaceColor','w', ...
            'Color', c_wI{wI}, 'LineStyle','none', ...
            'LineWidth', 1.0, 'CapSize', 3);

        % D22 drift prediction: colour AND line style distinguish waves
        y_theory = linspace(-2, 0, 400);
        z_line   = y_theory ./ k_case;
        [~, vz_ratio_d22] = profileD22SlipVelocity( ...
            z_line, H_case, T_case, h_case, d_p, rho_p, rho_w, nu, vs_term);
        plot(axp, -vz_ratio_d22, y_theory, ls_wI{wI}, ...
            'Color', c_wI{wI}, 'LineWidth', lw_d22);
    end

    xline(axp, 1, 'k-', 'LineWidth', 0.8, 'HandleVisibility','off');
end

% ---- zoom insets (near-unity region) ------------------------------------
% A grey box in the main axes marks the inset window; the inset repeats
% the D22 curves and any data points that fall inside the window.
drawnow;   % settle subplot positions before reading axp.Position
ax_in = gobjects(1,pN);
for pI = 1:pN
    axp = ax_vz(slotForPart(pI));

    % rectangle(axp, 'Position', ...
    %     [inset_xl, inset_yl, inset_xr-inset_xl, inset_yr-inset_yl], ...
    %     'EdgeColor', [0.45 0.45 0.45], 'LineWidth', 0.8);

    posp = axp.Position;
    ax_in(pI) = axes('Position', ...
        [posp(1)+0.08*posp(3), posp(2)+0.14*posp(4), ...
         0.28*posp(3), 0.37*posp(4)]);
    axIn = ax_in(pI);
    hold(axIn,'on'); box(axIn,'on');
    set(axIn,'FontSize',9,'Layer','top');
    ...
    for wI = 1:wN
        vs_term  = v_s(wI,pI);
        y_theory = linspace(inset_yl, inset_yr, 200);
        z_line   = y_theory ./ k(wI,pI);
        [~, vz_ratio_d22] = profileD22SlipVelocity( ...
            z_line, H(wI,pI), T(wI,pI), h(wI,pI), d_p, rho_p, rho_w, nu, vs_term);
        plot(axIn, -vz_ratio_d22, y_theory, ls_wI{wI}, ...
            'Color', c_wI{wI}, 'LineWidth', 1.2);

        % measurement points falling inside the zoom window
        ky_m  = kz_bin_z_mean{wI,pI};  vp_m  = kz_bin_vp_mean{wI,pI};
        ky_se = kz_bin_z_std{wI,pI};   vp_se = kz_bin_vp_std{wI,pI};
        if isempty(ky_m) || isempty(vp_m)
            continue
        end
        xw = vp_m ./ vs_term;  yw = ky_m;
        inWin = isfinite(xw) & isfinite(yw) & ...
                xw >= inset_xl & xw <= inset_xr & ...
                yw >= inset_yl & yw <= inset_yr;
        if any(inWin)
            errorbar(axIn, xw(inWin), yw(inWin), ...
                ciFactor*ky_se(inWin), ciFactor*ky_se(inWin), ...
                ciFactor*vp_se(inWin)./vs_term, ciFactor*vp_se(inWin)./vs_term, ...
                m_wI{wI}, 'MarkerSize', 4, 'MarkerFaceColor','w', ...
                'Color', c_wI{wI}, 'LineStyle','none', ...
                'LineWidth', 0.8, 'CapSize', 2);
        end
    end
    xline(axIn, 1, 'k-', 'LineWidth', 0.6);
    xlim(axIn, [inset_xl inset_xr]); ylim(axIn, [inset_yl inset_yr]);
    xticks(axIn, [inset_xl 1 inset_xr]);
    set(axIn,'XTickLabelRotation',0)
end
...
for wI = 1:wN
    Hleg2(2*wI-1) = plot(ax_leg2, nan, nan, m_wI{wI}, ...
        'MarkerSize', 8, 'MarkerFaceColor','w', 'Color', c_wI{wI}, ...
        'LineStyle','none', ...
        'DisplayName', sprintf('$\\widetilde{v_z}(z_p,t)\\,\\mathrm{(%s)}$',waves{wI}));

    Hleg2(2*wI) = plot(ax_leg2, nan, nan, ls_wI{wI}, ...
        'Color', c_wI{wI}, 'LineWidth', lw_d22, ...
        'DisplayName', sprintf('$v_{z-drift}$ (%s)', waves{wI}));
end
legend(ax_leg2, Hleg2, 'FontSize', 20, 'Location', 'northwest', 'Interpreter','latex');
...
% ---- vector PDF export -----------------------------------------------------
outpdf_vz = 'E:\2.ParticlesInWave\Manuscript\figures\waveavg_vz_profile.pdf';
exportgraphics(fig_vz, outpdf_vz, 'ContentType','vector', 'BackgroundColor','none');
...
movwin = 125;   % 0.5 s moving mean for the displayed trial time series
...
% [r5] representation:
%   - trials: FULL instantaneous time series (0.5 s moving mean), drawn
%     in a uniform light tint of the wave colour (opaque, vector-safe;
%     no gradient, no transparency).
%   - ensemble: computed in the WAVE-AVERAGED sense. Each trial's force
%     ratios are first averaged over consecutive full wave periods
%     (t in [2*pi*(n-1), 2*pi*n)); the ensemble mean over trials is then
%     taken per period index (>= nMin trials) and drawn as a SOLID line
%     with filled markers at the period-centre points, in the full wave
%     colour. Direct ensemble-averaging of the raw time series is
%     avoided because trials are released at different wave phases.
Tper     = 2*pi;   % one wave period in dimensionless time t = omega*t'
covMin   = 0.9;    % min fraction of a period that must be sampled
nMin     = 3;      % min number of trials for the ensemble mean
tint_w   = 0.6;   % white fraction of the trial tint (0 = base colour)
lw_trial = 0.6;    % trial line width
lw_mean  = 1.8;    % ensemble line width
ms_mean  = 4.5;    % ensemble marker size
lw_ref   = 1.4;    % reference (5*omega*tau_p) line width
dsamp    = 2;      % plot every dsamp-th sample of each trial curve

lighten = @(c) (1-tint_w)*c + tint_w*[1 1 1];

% === Create figure ===
figR = figure('Name','R_D & R_{wave} - all particles');
set(figR,'Position',[-500 -300 1100 1200]);
set(figR,'Color','w');
set(figR,'Renderer','painters');   % vector-safe: no transparency used
...
for pI = 1:nPart
    ...
    % storage for the per-wave ensemble curves (plotted on top afterwards)
    tbar_w  = cell(1,nWave);
    RDens_w = cell(1,nWave);
    RWens_w = cell(1,nWave);
    omega_w = NaN(1,nWave);
    ...
    for wI = 1:nWave
        ...
        trialCol = lighten(waveColorMap.(wave));   % uniform light tint

        % ---- pass 1: full trial time series (light) + per-period averages
        RD_per = NaN(case_tot, 0);   % (trial, period index), grown as needed
        RW_per = NaN(case_tot, 0);
        ...
        for j = 1:case_tot
            ...
            % ---- instantaneous R_drag(t)
            Urel  = hypot(u_sl,v_sl);
            Re_t  = Urel * d_p / nu;
            SchN  = 1 + 0.15 * Re_t.^0.687;
            R_D_t = abs(3*pi*mu*d_p .* SchN .* v_sl) / W_sub;

            % ---- instantaneous R_wave(t): five-point stencil on v_f
            dvfdt = nan(N,1);
            for n = 3:N-2
            ...
            % ---- trial curves: full time series, moving-mean smoothed
            RDs = movmean(R_D_t,    movwin);
            RWs = movmean(R_wave_t, movwin);

            plot(ax_RD, t_nd(1:dsamp:end), RDs(1:dsamp:end), '-', ...
                'Color', trialCol, 'LineWidth', lw_trial);
            plot(ax_RW, t_nd(1:dsamp:end), RWs(1:dsamp:end), '-', ...
                'Color', trialCol, 'LineWidth', lw_trial);

            % ---- per-period (wave-averaged) values from the RAW series
            nper = floor(t_nd(end) / Tper);
            if nper < 1
                continue;
            end

            nSampFull = Tper / (omega*dt);        % samples in a full period
            RDbar_j = NaN(1,nper);
            RWbar_j = NaN(1,nper);

            for n = 1:nper
                in = t_nd >= (n-1)*Tper & t_nd < n*Tper;
                if nnz(in) < covMin*nSampFull
                    continue;                     % poorly sampled period
                end
                RDbar_j(n) = mean(R_D_t(in),   'omitnan');
                RWbar_j(n) = mean(R_wave_t(in),'omitnan');
            end

            if nper > size(RD_per,2)
                RD_per(:, end+1:nper) = NaN;
                RW_per(:, end+1:nper) = NaN;
            end
            RD_per(j,1:nper) = RDbar_j;
            RW_per(j,1:nper) = RWbar_j;
        end

        % ---- ensemble mean of the wave-averaged values, per period index
        if isempty(RD_per)
            continue;
        end
        nOK_RD = sum(isfinite(RD_per), 1);
        nOK_RW = sum(isfinite(RW_per), 1);
        RD_ens = mean(RD_per, 1, 'omitnan');  RD_ens(nOK_RD < nMin) = NaN;
        RW_ens = mean(RW_per, 1, 'omitnan');  RW_ens(nOK_RW < nMin) = NaN;

        nperMax = size(RD_per,2);
        tbar_w{wI}  = ((1:nperMax) - 0.5) * Tper;   % period-centre times
        RDens_w{wI} = RD_ens;
        RWens_w{wI} = RW_ens;
    end

    % ---- pass 2: ensemble curves on top of all trial bundles
    %      solid line through the wave-averaged ensemble points,
    %      filled markers at the period centres
    for wI = 1:nWave
        if isempty(tbar_w{wI}), continue; end
        baseCol = waveColorMap.(waves{wI});
        plot(ax_RD, tbar_w{wI}, RDens_w{wI}, '-', ...
            'Color', baseCol, 'LineWidth', lw_mean, ...
            'Marker','o', 'MarkerSize', ms_mean, 'MarkerFaceColor', baseCol);
        plot(ax_RW, tbar_w{wI}, RWens_w{wI}, '-', ...
            'Color', baseCol, 'LineWidth', lw_mean, ...
            'Marker','o', 'MarkerSize', ms_mean, 'MarkerFaceColor', baseCol);
    end

    % ---- reference lines last, so they stay visible over the bundle
    for wI = 1:nWave
        if isnan(omega_w(wI)), continue; end
        baseCol = waveColorMap.(waves{wI});
        xline(ax_RD, 5*omega_w(wI)*tau_p, '--', 'Color', baseCol, 'LineWidth', lw_ref);
        xline(ax_RW, 5*omega_w(wI)*tau_p, '--', 'Color', baseCol, 'LineWidth', lw_ref);
    end
    yline(ax_RD, 1, 'k-','LineWidth',1.2)

    grid(ax_RD,'on');  grid(ax_RW,'on');
    if pI == nPart
...
% --- Legend: solid line + filled marker = wave-averaged ensemble mean,
%     thin light = individual trial time series ---
ax_legend = subplot(8,2,2); hold(ax_legend,'on');
p1 = plot(ax_legend, nan,nan, '-o', 'Color', waveColorMap.W1, 'MarkerFaceColor', waveColorMap.W1, ...
    'MarkerSize', ms_mean, 'LineWidth',lw_mean, 'DisplayName','W1');
p2 = plot(ax_legend, nan,nan, '-o', 'Color', waveColorMap.W2, 'MarkerFaceColor', waveColorMap.W2, ...
    'MarkerSize', ms_mean, 'LineWidth',lw_mean, 'DisplayName','W2');
p3 = plot(ax_legend, nan,nan, '-o', 'Color', waveColorMap.W3, 'MarkerFaceColor', waveColorMap.W3, ...
    'MarkerSize', ms_mean, 'LineWidth',lw_mean, 'DisplayName','W3');
% p4 = plot(ax_legend, nan,nan, '-',  'Color', lighten([0.2 0.2 0.2]), 'LineWidth',lw_trial, 'DisplayName','trial');
% p5 = plot(ax_legend, nan,nan, '-o', 'Color', [0.20 0.20 0.20], 'MarkerFaceColor', [0.20 0.20 0.20], ...
%     'MarkerSize', ms_mean, 'LineWidth',lw_mean, 'DisplayName','wave-avg. ensemble');
legend(ax_legend,[p1 p2 p3],'Location','northeast','NumColumns',3);

% --- Save as vector PDF ---
outpdf = 'E:\2.ParticlesInWave\Manuscript\figures\force_ratio.pdf';
exportgraphics(figR, outpdf, 'ContentType','vector', 'BackgroundColor','none');
...
% NOTE [r2]: the shaded SE bands use FaceAlpha; exportgraphics keeps them
% in vector PDF on R2022a+ . If your release rasterizes them, replace the
% FaceAlpha fills with pre-blended opaque light colours.
...
function colj = tintColor(baseCol, j, n)
    % [r2] opaque, vector-safe substitute for line alpha (figure 9):
    % mix the base colour with white; later trials are darker.
    w = 0.70 - 0.45*(j-1)/max(1,n-1);   % white fraction: 0.70 -> 0.25
    colj = (1-w)*baseCol + w*[1 1 1];
end

...
for wI = 1:wN
  ...
  for pI = 1:pN
    ...
    S = load(fullfile([wave partinfo], sprintf('%s%s_vs5_avg_unit.mat', wave, partinfo)));
...
for wI = 1:wN
    if wI == 1
        HH(wI) = scatter(off_exp(wI,1:end-3), -off_theo(wI,1:end-3), msz, Ga(wI,1:end-3), ...
...
for wI = 1:wN
    if wI == 1
        HH(wI) = scatter(Rz_exp(wI,1:end-3), Rz_theo(wI,1:end-3), msz, Ga(wI,1:end-3), ...
                        'filled', 'Marker', markers{wI}, 'DisplayName', waves{wI});
    else
...
for wI = 1:wN
    if wI==1
        HH(wI) = scatter(St(wI,1:end-3), phi_diff_z(wI,1:end-3), msz, Ga(wI,1:end-3), ...
                    'filled', 'Marker', markers{wI}, 'DisplayName', waves{wI});
    else

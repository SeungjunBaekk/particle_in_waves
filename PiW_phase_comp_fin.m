...
legend(axL, [hpp2; hpp3], 'Location','best','Interpreter','latex','FontSize',16);
...
legend(axL, [hq3; hq4], 'Location','best','Interpreter','latex','FontSize',16);
...
% NOTE [r2]: the CI bands use fill(...,'FaceAlpha',0.5); exportgraphics
% keeps the transparency in vector PDF on R2022a+ . If your release
% rasterizes them, pre-blend the band colours to opaque instead.
...
%% theory vs. measurements (figure 13)
...
mfa    = 0.65;   % marker face alpha
msMain = 55;     % standard marker size
msSpec = 55;     % W1-affected (diamond) marker size
msz0   = 10;     % (c,f): rho*-scaled size, base
mszAmp = 200;     % (c,f): rho*-scaled size, amplitude  ->  ms = msz0 + mszAmp*rho*

GaLim     = [0 400];
cmap_true = turbo(400);                    % marker colours
cmap_disp = mfa*cmap_true + (1-mfa);       % colorbar colours (pre-blended)
gaColor   = @(v) cmap_true( max(1, min(size(cmap_true,1), ...
    1 + round((v-GaLim(1))/(GaLim(2)-GaLim(1))*(size(cmap_true,1)-1)))), :);
...
for wI = 1:wN
    for pI = 1:pN
        ...
        if isfinite(offx_exp(wI,pI)) && isfinite(offx_theo(wI,pI)) && isfinite(Ga(wI,pI))
            ...
            scatter(ax1, offx_exp(wI,pI), offx_theo(wI,pI), ms, gaColor(Ga(wI,pI)), ...
                'filled', 'Marker', mk, 'MarkerEdgeColor','k', 'LineWidth',lw, 'MarkerFaceAlpha',mfa);
        end
    end
end
...
for wI = 1:wN
    for pI = 1:pN
        ...
        if isfinite(Rx_exp(wI,pI)) && isfinite(Rx_theo(wI,pI)) && isfinite(Ga(wI,pI))
            ...
            scatter(ax2, Rx_exp(wI,pI), Rx_theo(wI,pI), ms, gaColor(Ga(wI,pI)), ...
                'filled', 'Marker', mk, 'MarkerEdgeColor','k', 'LineWidth',lw, 'MarkerFaceAlpha',mfa);
        end
    end
end
    ...
    for pI = 1:pN
        ...
        if isfinite(St(wI,pI)) && isfinite(delta_x_star(wI,pI)) && ...
           isfinite(rho_x_star(wI,pI)) && isfinite(Ga(wI,pI))
            ...
            ms = msz0 + mszAmp * max(0, min(1, rho_x_star(wI,pI)));   % size ~ rho*_x

            scatter(ax3, St(wI,pI), delta_x_star(wI,pI), ms, gaColor(Ga(wI,pI)), ...
                'filled', 'Marker', mk, 'MarkerEdgeColor','k', 'LineWidth',lw, 'MarkerFaceAlpha',mfa);
        end
    end
end

text(ax3, 0.6, 0.93, '$\mathrm{size}\propto \rho_x^\ast$', ...
...
for wI = 1:wN
    for pI = 1:pN
        ...
        if isfinite(offz_exp(wI,pI)) && isfinite(offz_theo(wI,pI)) && isfinite(Ga(wI,pI))
            ...
            scatter(ax4, offz_exp(wI,pI), offz_theo(wI,pI), ms, gaColor(Ga(wI,pI)), ...
                'filled', 'Marker', mk, 'MarkerEdgeColor','k', 'LineWidth',lw, 'MarkerFaceAlpha',mfa);
        end
    end
end
...
for wI = 1:wN
    for pI = 1:pN
        ...
        if isfinite(Rz_exp(wI,pI)) && isfinite(Rz_theo(wI,pI)) && isfinite(Ga(wI,pI))
            ...
            scatter(ax5, Rz_exp(wI,pI), Rz_theo(wI,pI), ms, gaColor(Ga(wI,pI)), ...
                'filled', 'Marker', mk, 'MarkerEdgeColor','k', 'LineWidth',lw, 'MarkerFaceAlpha',mfa);
        end
    end
end
    ...
    for pI = 1:pN
        ...
        if isfinite(St(wI,pI)) && isfinite(delta_z_star(wI,pI)) && ...
           isfinite(rho_z_star(wI,pI)) && isfinite(Ga(wI,pI))
            ...
            ms = msz0 + mszAmp * max(0, min(1, rho_z_star(wI,pI)));   % size ~ rho*_z

            scatter(ax6, St(wI,pI), delta_z_star(wI,pI), ms, gaColor(Ga(wI,pI)), ...
                'filled', 'Marker', mk, 'MarkerEdgeColor','k', 'LineWidth',lw, 'MarkerFaceAlpha',mfa);
        end
    end
end

text(ax6, 0.6, 0.93, '$\mathrm{size}\propto \rho_z^\ast$', ...
    'Units','normalized','Interpreter','latex','FontSize',12);
...
rx = rho_x_star(use_case & isfinite(rho_x_star));
rz = rho_z_star(use_case & isfinite(rho_z_star));
fprintf('rho*_x: min %.2f / median %.2f / max %.2f\n', min(rx), median(rx), max(rx));
fprintf('rho*_z: min %.2f / median %.2f / max %.2f\n', min(rz), median(rz), max(rz));
